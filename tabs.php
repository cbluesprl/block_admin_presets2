...
$adminpresetsurl = $CFG->wwwroot . '/blocks/admin_presets2/index.php';
...
if (!array_key_exists($this->action, $adminpresetstabs)) {
    $row[] = new tabobject($this->action, $adminpresetsurl .
            '?action=' . $this->action, get_string('action' . $this->action, 'block_admin_presets2'));
}

foreach ($adminpresetstabs as $actionname) {
    $row[] = new tabobject($actionname, $adminpresetsurl .
            '?action=' . $actionname, get_string('action' . $actionname, 'block_admin_presets2'));
}
