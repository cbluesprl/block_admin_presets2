...
class automatic_export_cleanup extends scheduled_task {
    ...
    public function get_name() {
        return get_string('automaticexportcleanup', 'block_admin_presets2');
    }
    ...
    public function execute() {
        ...
        $config = get_config('block_admin_presets2', 'automaticexportcleanup');
        if (empty($config)) {
            return;
        }
    ...
    private function delete($record) {
        ...
        if (!$DB->delete_records('block_admin_presets', array('id' => $record->id))) {
            print_error('errordeleting', 'block_admin_presets2');
        }
