...
require_once($CFG->dirroot . '/blocks/admin_presets2/lib/admin_presets_settings_types.php');
...
class admin_presets_base {
        ...
        if ($presets) {
            ...
            foreach ($presets as $preset) {
                ...
                $previewlink = $CFG->wwwroot .
                        '/blocks/admin_presets2/index.php?action=load&mode=preview&id=' . $preset->id;
                $loadlink = $CFG->wwwroot .
                        '/blocks/admin_presets2/index.php?action=load&id=' . $preset->id;
                $downloadlink = $CFG->wwwroot .
                        '/blocks/admin_presets2/index.php?action=export&mode=download_xml&sesskey=' .
                        sesskey() . '&id=' . $preset->id;
                $deletelink = $CFG->wwwroot .
                        '/blocks/admin_presets2/index.php?action=delete&id=' . $preset->id;
                $rollbacklink = $CFG->wwwroot .
                        '/blocks/admin_presets2/index.php?action=rollback&id=' . $preset->id;
                ...
                if ($DB->get_records('block_admin_presets_app', array('adminpresetid' => $preset->id))) {
                    $actions[] = html_writer::link($rollbacklink,
                            get_string("rollback", "block_admin_presets2"));
                }
            ...
            $exportlink = '<a href="' . $CFG->wwwroot .
                    '/blocks/admin_presets2/index.php?action=export">' .
                    strtolower(get_string("actionexport", "block_admin_presets2")) . '</a>';
            $importlink = '<a href="' .
                    $CFG->wwwroot . '/blocks/admin_presets2/index.php?action=import">' .
                    strtolower(get_string("actionimport", "block_admin_presets2")) . '</a>';

            $this->outputs = $OUTPUT->box_start('generalbox', 'id_nopresets');
            $this->outputs .= '<ul>' . get_string('nopresets', 'block_admin_presets2');
            $this->outputs .= '<li>' . get_string('toexportclick',
                            'block_admin_presets2', $exportlink) . '</li>';
            $this->outputs .= '<li>' . get_string('toimportclick',
                            'block_admin_presets2', $importlink) . '</li>';
            $this->outputs .= '</ul>';
    ...
    protected function _create_preset_data_table($actionstable = true) {
        ...
        $table->head = array(get_string('name'), get_string('description'),
                get_string('presetmoodlerelease', 'block_admin_presets2'),
                get_string('author', 'block_admin_presets2'),
                get_string('site', 'block_admin_presets2'),
                get_string('created', 'block_admin_presets2'),
                get_string('imported', 'block_admin_presets2'));
    ...
    protected function _display_header() {
        ...
        // Strings.
        $actionstr = get_string('action' . $this->action, 'block_admin_presets2');
        $modestr = get_string($this->action . $this->mode, 'block_admin_presets2');
        $titlestr = get_string('pluginname', 'block_admin_presets2');

        // Header.
        $PAGE->set_title($titlestr);
        $PAGE->set_heading($SITE->fullname);

        $PAGE->navbar->add(get_string('pluginname', 'block_admin_presets2'),
                new moodle_url($CFG->wwwroot . '/blocks/admin_presets2/index.php'));
    ...
    public function log() {
        ...
        if ($this->mode != 'show' ||
            ...
            $eventnamespace = '\\block_admin_presets2\\event\\' . self::$eventsactionsmap[$action];
            $eventdata = array(
    ...
    protected function _get_settings_branches($settings) {
        ...
        $PAGE->requires->js_init_call('M.block_admin_presets2.init', null, true);
        ...
        $PAGE->requires->js_init_call('M.block_admin_presets2.addNodes',
                array($ids, $nodes, $labels, $descriptions, $parents), true);
        $PAGE->requires->js_init_call('M.block_admin_presets2.render', null, true);
    }
    ...
    protected function _output_applied_changes($appliedchanges) {
        ...
        $appliedtable->head = array(get_string('plugin'),
                get_string('settingname', 'block_admin_presets2'),
                get_string('oldvalue', 'block_admin_presets2'),
                get_string('newvalue', 'block_admin_presets2'));
