...
class admin_presets_import extends admin_presets_base {
    ...
    public function show() {
        ...
        $url = $CFG->wwwroot . '/blocks/admin_presets2/index.php?action=import&mode=execute';
    ...
    public function execute() {
        ...
        if ($data = $this->moodleform->get_data()) {
            ...
            if (!$xml) {
                redirect($CFG->wwwroot . '/blocks/admin_presets2/index.php?action=import',
                        get_string('wrongfile', 'block_admin_presets2'), 4);
            }
            ...
            if (empty($settingsfound)) {
                ...
                redirect($CFG->wwwroot . '/blocks/admin_presets2/index.php?action=import',
                        get_string('novalidsettings', 'block_admin_presets2'), 4);
            }

            // Trigger the as it is usually triggered after execute finishes.
            $this->log();

            redirect($CFG->wwwroot . '/blocks/admin_presets2/index.php?action=load&id=' . $preset->id);
        }
    }
}
